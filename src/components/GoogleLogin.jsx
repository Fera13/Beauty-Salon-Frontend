...
function GoogleLogin(onLoginSuccess) {
  async function handleCredentialResponse(userData) {
    ...
    try {
      const response = await fetch(
        "https://beauty-salon-back.onrender.com/users/register",
        {
          method: "POST",
    ...
    try {
      const response = await fetch(
        "https://beauty-salon-back.onrender.com/users/login",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  function initializeGoogleLogin() {
    google.accounts.id.initialize({
      client_id:
        "1079428828720-dfo4k6av3jvepch0hmpums7a1agal8dl.apps.googleusercontent.com", // belongs in .env file but works for now
